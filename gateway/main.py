...
Known gaps: /health doesn't actually check Ollama reachability yet.
...
def _stream_response(model, messages, images, swap_info, request_start, has_image):
    try:
...
def chat_completions(req: ChatCompletionRequest):
    ...
    if req.stream:
        return StreamingResponse(
            _stream_response(
                model, messages, images, swap_info, request_start, has_image
            ),
...
def health():
    ...
    try:
        ...
        return JSONResponse(
            status_code=503,
            content={"status": "degraded", "ollama_reachable": False, "detail": str(e)},
        )
