...
const ConversationsSection = memo(() => {
    ...
    const { handleNewChatClick } = useNewChat({ onNewChat: () => toggleNav() });
            ...
            <div className="px-3 pb-2">
                <div className="mb-3 flex items-center gap-2 px-1">
                    <img
                        ...
                        alt={localize('com_ui_logo', { 0: startupConfig?.appTitle ?? 'Clarisync' })}
                    />
                    <span
                        className="truncate text-base font-semibold text-text-primary"
                        style={{ fontFamily: 'Poppins, Inter, system-ui, sans-serif' }}
                    >
                        {startupConfig?.appTitle ?? 'Clarisync'}
                    </span>
                </div>
                <button
                    type="button"
                    ...
                    className="flex w-full items-center gap-2 rounded-lg border border-border-medium px-3 py-2 text-sm font-medium text-text-primary transition-colors hover:bg-surface-hover"
                >
